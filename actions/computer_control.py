...
# MARK XXX — Computer Control (macOS-compatible)
#
# Changes from Windows version:
#   - _focus_window() uses AppleScript instead of PowerShell
#   - Hotkey helpers use 'command' instead of 'ctrl' where appropriate
#   - cv2.CAP_DSHOW replaced with cv2.CAP_AVFOUNDATION on macOS
...
def generate_random_data(data_type: str) -> str:
    ...
    if dt == "address":
        return f"{random.randint(100,9999)} {random.choice(['Main St','Oak Ave','Park Blvd','Elm St'])}"
...
def _focus_window(title: str) -> str:
    """Bring a window to focus by title — macOS uses AppleScript."""
    if _OS == "Darwin":
        try:
            ...
            # Fallback: use System Events to find by window name
            try:
                script = (
                ...
                return f"Could not focus window: {e2}"
    elif _OS == "Windows":
        try:
    ...
    return f"Window focus not supported on {_OS}"


def _select_all() -> str:
    mod = "command" if _OS == "Darwin" else "ctrl"
    return _hotkey(mod, "a")
...
def computer_control(
    ...
    try:
        if action == "type":
            ...
            return _click(x=parameters.get("x"), y=parameters.get("y"),
            ...
            return _move_mouse(int(parameters.get("x", 0)), int(parameters.get("y", 0)),
                               float(parameters.get("duration", 0.3)))

        elif action == "drag":
            return _drag(int(parameters.get("x1", 0)), int(parameters.get("y1", 0)),
            ...
            result = generate_random_data(parameters.get("type", "name"))
            print(f"[ComputerControl] 🎲 Random {parameters.get('type')}: {result}")
            return result
