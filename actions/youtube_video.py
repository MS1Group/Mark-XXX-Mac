...
def open_browser():
    ...
    if _os == "Darwin":
        ...
        for browser in ["google-chrome", "chromium-browser", "firefox"]:
            try:
...
def find_video_thumbnails() -> list[tuple[int, int]]:
    ...
    try:
        if _IMAGEGRAB_OK and platform.system() == "Windows":
            screenshot = ImageGrab.grab()
        else:
            import mss, PIL.Image
            with mss.mss() as sct:
                raw = sct.grab(sct.monitors[1])
                screenshot = PIL.Image.frombytes("RGB", raw.size, raw.bgra, "raw", "BGRX")
