...
def _get_camera_index() -> int:
    ...
    for idx in range(6):
        cap = cv2.VideoCapture(idx, cv2.CAP_AVFOUNDATION if __import__("platform").system() == "Darwin" else (cv2.CAP_DSHOW if __import__("sys").platform == "win32" else cv2.CAP_V4L2))
        if not cap.isOpened():
            cap.release()
            continue
...
def _capture_camera() -> bytes:
    ...
    cap = cv2.VideoCapture(camera_index, cv2.CAP_AVFOUNDATION if __import__("platform").system() == "Darwin" else (cv2.CAP_DSHOW if __import__("sys").platform == "win32" else cv2.CAP_V4L2))
    if not cap.isOpened():
