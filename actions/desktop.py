# actions/desktop.py
# macOS-compatible desktop & wallpaper management
# Removed: ctypes, winreg — replaced with osascript/subprocess
...
def set_wallpaper(image_path: str) -> str:
    ...
    if path.suffix.lower() not in [".jpg", ".jpeg", ".png", ".bmp", ".heic"]:
        return f"Unsupported format: {path.suffix}. Use jpg, png, bmp, or heic."
...
def get_current_wallpaper() -> str:
    try:
        if _OS == "Darwin":
            script = 'tell application "Finder" to get POSIX path of (get desktop picture)'
            result = subprocess.run(["osascript", "-e", script], capture_output=True, text=True)
            path = result.stdout.strip()
            return f"Current wallpaper: {path}" if path else "Could not retrieve wallpaper path."
...
def organize_desktop(mode: str = "by_type") -> str:
    ...
    for item in desktop.iterdir():
        if item.is_dir() or item.name.startswith("."):
            continue
        if item.suffix.lower() in [".lnk", ".alias"]:
            continue
    ...
    try:
        if action == "wallpaper":
            ...
            if full_task:
                ...
                result = _execute_generated_code(code) if code != "UNSAFE" else "Cannot do that safely."
            else:
